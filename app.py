...
def index():
    if 'username' in session:
        ...
        total_amount = sum(transaction[2] for transaction in expenses)
        total_upi = sum(transaction[2] for transaction in expenses if transaction[6] == 'UPI')
        total_cash = sum(transaction[2] for transaction in expenses if transaction[6] == 'Cash')
...
def daily_spending_data():
    if 'username' in session:
        ...
        conn = sqlite3.connect(DATABASE)
        c = conn.cursor()
        c.execute("SELECT date, SUM(amount) FROM expenses WHERE user_id = ? GROUP BY date", (user_id,))
        data = c.fetchall()
        conn.close()
...
def monthly_spending_data():
    if 'username' in session:
        ...
        conn = sqlite3.connect(DATABASE)
        c = conn.cursor()
        c.execute("SELECT strftime('%Y-%m', date) AS month, SUM(amount) FROM expenses WHERE user_id = ? GROUP BY month", (user_id,))
        data = c.fetchall()
        conn.close()
...
def statistics():
    ...
    if user_id:
        ...
        c.execute("SELECT category, SUM(amount) FROM expenses WHERE user_id = ? GROUP BY category ORDER BY SUM(amount) DESC LIMIT 5", (user_id,))
